...
fn run_tui(
    ...
    app.status = startup_note;
    {
        ...
        app.push_system(guard.startup_briefing());
        // F6 ölçüm zincirinin son halkası: bir ölçüm yapıldı ama kimse bakmıyorsa zincir kopuk.
        // Model veya prompt değiştiğinde bunu hatırlamak insana bırakılan bir disiplindi ve
        // kaçınılmaz olarak unutuluyordu; artık araç hatırlatıyor.
        if let Some(notice) = guard.unmeasured_configuration_notice(provider.model_id()) {
...
fn submit(
    ...
    match input.as_str() {
        ...
        "/status" => {
            ...
            let measurement = {
                let guard = runtime.lock().expect("JARVIS runtime lock poisoned");
                match guard.configuration_is_measured(provider.model_id()) {
                    Ok(true) => "ölçüldü".to_string(),
                    Ok(false) => "ÖLÇÜLMEDİ (/eval)".to_string(),
                    Err(_) => "ölçüm kaydı yok".to_string(),
                }
            };
            app.push_system(format!(
