...
A request to write, create, or generate new code, a script, or a program (Turkish \"kod yaz\", \"script yaz\", \"program yaz\", \"yazar mısın\") is creative/coding work, even if it also asks to save or put the result directly somewhere — it is never file.read_workspace, project.info, code.project_outline, or docs.workspace_summary (those only ever describe files that already exist, they cannot write anything new), and it is never note.create either (a note is a short piece of information to remember, not source code); route it UNKNOWN. \
