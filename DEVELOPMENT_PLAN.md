...
| F4 | Onaylı, izole coding ve yerel iş workbench'i | İLERLİYOR (11/15 madde `[x]`: plan(varsayım/soru/risk dahil)→patch→onay→uygula→test zinciri, taban çizgili regresyon tespiti + seçilebilir dosya scope'lu patch preview dahil, uçtan uca TUI'de çalışıyor, 7 senaryolu eval seti geçiyor, `LocalTool` çerçevesi 2 gerçek tool'la kanıtlandı; çok-adımlı workflow runner ve OS-seviyesi izolasyon (seccomp/cgroups/overlay) hâlâ açık) | F2 + OS-isolated worker |
