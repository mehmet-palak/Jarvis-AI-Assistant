...
- [x] Aktif keşif (yalnız scope onaylıysa) — port/servis tarama + subdomain brute-force (kalan alt madde açık: JS analiziyle endpoint keşfi).
  ...
  - **Kalan:** Bu ikisi de F7.2'nin SOCKS5 kapısından DEĞİL, doğrudan JARVIS'in kendi sürecinden bağlanıyor — henüz sandbox'lı bir pentest worker'ı olmadığı için (bu, F7.2'nin kendi kalan maddesi). Servis parmak izi/banner alma ve JS analiziyle endpoint keşfi burada yok, ayrı alt maddeler.
  - **Test:** 7 (port tarama) + 6 (subdomain brute-force: aday üretimi, varsayılan liste, mod tavanı reddi, boş/aşırı liste reddi, aktif scope zorunluluğu) + 1 gerçek-DNS testi (`#[ignore]`, elle koşulup doğrulandı) = 14 yeni test.
