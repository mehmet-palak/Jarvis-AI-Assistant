...
- [x] Network-scoped sandbox worker'ın temel taşı: allowlist egress gate.
  ...
  - **Kalan (henüz yapılmadı):** Bu gate'in `isolated_worker_command`'a gerçekten bağlanması (sandboxed bir pentest worker'ın bu proxy'yi kullanacak şekilde başlatılması) — şu an bağımsız, kendi başına doğrulanmış bir bileşen, henüz bir worker'a takılmadı. Bu, ayrı bir tasarım kararı gerektiriyor (`--unshare-net` altındaki bir worker, host'un loopback'indeki bu kapıya nasıl ulaşacak — ör. bir Unix domain socket bind-mount'u) ve bilinçli olarak ertelendi.
