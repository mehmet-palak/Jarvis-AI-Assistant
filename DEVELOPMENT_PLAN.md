...
Durum: **MOBİL HARİÇ KAPATILDI (21 Ağustos 2026).** Kullanıcı F9 bittikten sonra F8'e geçti ve "f8'i bugün mobil hariç kapatıyoruz" dedi. Kalan maddeleri incelerken mimari bir gerçek çıktı: F8'in ağ/güven gerektiren kısımları (MCP egress, OAuth e-posta, imzalı eklenti sandbox'ı) hepsi **tek bir güven mekanizmasını** paylaşıyor ve mobil ile aynı "önce güvenilir çekirdek" kapısına tabi — bu yüzden [ADR-0008](docs/adr/0008-mcp-architecture-and-security-layers.md)'de bir kez, doğru tasarlanıp **F10'da** uygulanacak. Şimdi yapılabilir tek şey olan **yerel, ağsız entegrasyon** (takvim) tamamlandı. Mobil (madde 4-5) kullanıcının açık kuralıyla beklemede.

- [~] MCP production hardening — protocol sürümleme + credential/raw-secret response filtresi tamam; **kalan egress + tüm sertleştirme katmanları [ADR-0008](docs/adr/0008-mcp-architecture-and-security-layers.md) ile tasarlandı ve F10'a ertelendi** (manifest imzası, sampling deny-by-default, tool poisoning, döngü bileşimi, tedarik zinciri/rug-pull, sunucular-arası kaçırma zinciri — hepsi mevcut omurga parçalarının yeniden kullanımı, ayrı ayrı kurulmasın diye F10'da bir kez uygulanacak).
...
- [ ] Plugin/skill ekosistemi: signed/allowlisted paketler, capability sandbox profile, sürüm uyumluluğu, kilitleme dosyası ve tek tıkla devre dışı bırakma. **F10'a erteli:** bu, mimari olarak MCP dış-araç güven modelinin (ADR-0008 Katman 1-3) birebir aynısı — bir eklenti teknik olarak izole, imzalı bir dış araçtır. Ayrı bir sistem olarak kurmak F10'a ertelenen güven çekirdeğini kopyalamak olurdu; tek mekanizma olarak F10'da kurulacak.
- [ ] Remote/mobile yalnız explicit device pairing, public key, nonce/replay koruması, revoke, bağlantı şifreleme ve server-side kill switch'ten sonra ele alınır. **Kullanıcının açık kuralıyla beklemede** ("ben diyene kadar mobile girişmiyoruz").
- [ ] Cross-device handoff: task scope'u genişletmeyen typed handoff, offline queue/conflict policy ve alıcı cihazda yeniden policy değerlendirmesi. **Mobil ile birlikte beklemede.**

Tamamlanma ölçütü: Yerel desktop sürümü güvenilir olmadan hiçbir eklenti, entegrasyon veya remote cihaz tool yetkisi ya da kişisel bellek erişimi almaz. **(F8 ağsız kapsamı 21 Ağustos 2026'da bu ölçütle kapatıldı; ağ/güven maddeleri ADR-0008 ile F10'a taşındı, mobil kullanıcı onayıyla açılacak.)**
