...
| F4 | Onaylı, izole coding ve yerel iş workbench'i | İLERLİYOR (12/15 madde `[x]`: plan(varsayım/soru/risk dahil)→patch→onay→uygula→test zinciri, taban çizgili regresyon tespiti + seçilebilir dosya scope'lu patch preview dahil, uçtan uca TUI'de çalışıyor, 7 senaryolu eval seti geçiyor, `LocalTool` çerçevesi 2 gerçek tool'la ve genel bir `workflow.rs` çok-adımlı orkestratörü kanıtlandı; gerçek cgroup v2 (RAM/CPU) eklendi, izole worker (bwrap+cgroup) ilk kez gerçek makinede uçtan uca kanıtlandı — 2 gerçek bug (RLIMIT_NPROC per-UID yanlış hesap, tmpfs/bind mount sırası) bulunup düzeltildi; kalan seccomp/gerçek overlay artık "burada yapılamaz" değil, yalnız "henüz yapılmadı") | F2 + OS-isolated worker |
...
- [ ] Isolated worker bootstrap: workspace snapshot/overlay, ayrı çalışma dizini, read-only input ve sınırlandırılmış output artifact alanı. (16 Ağustos 2026, 10. tur: gerçek makinede `bwrap --tmp-overlay` ayrıcalıksız çalıştığı doğrulandı — daha önce "bu sandbox'ta doğrulanamaz" denilen şey yanlıştı, bkz. aşağıdaki düzeltme notu. Henüz uygulanmadı.)
- [ ] OS izolasyonu: kullanıcı namespace/container/bubblewrap seçimi, ağ=kapalı varsayılanı, mount allowlist, no-new-privileges ve seccomp fizibilitesi. (aynı düzeltme: seccomp burada gerçekten denenebilir, henüz yapılmadı.)
- [ ] Resource kontrolü: CPU/RAM/disk/PID/time quota, process group, watchdog, stdout/stderr limitleri ve güvenli cleanup. (16 Ağustos 2026, 10. tur: CPU/RAM/PID/time/cleanup artık **çift katmanlı gerçek** — hem `setrlimit` hem gerçek cgroup v2 (`systemd-run --user --scope -p MemoryMax/-p CPUQuota`), ikisi de canlı OOM-kill ile kanıtlandı. Yalnız toplam disk kotası (cgroups bunu sağlamıyor, overlay/tmpfs `size=` gerekir) eksik — bu yüzden madde hâlâ `[ ]`.)
