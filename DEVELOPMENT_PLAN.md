...
| F4 | Onaylı, izole coding ve yerel iş workbench'i | İLERLİYOR (14/15 madde `[x]`: plan(varsayım/soru/risk dahil)→patch→onay→uygula→test zinciri, taban çizgili regresyon tespiti + seçilebilir dosya scope'lu patch preview dahil, uçtan uca TUI'de çalışıyor, 7 senaryolu eval seti geçiyor, `LocalTool` çerçevesi 2 gerçek tool'la ve genel bir `workflow.rs` çok-adımlı orkestratörü kanıtlandı; gerçek cgroup v2 (RAM/CPU) + `WorkspaceWriteMode::Overlay` (disk bütçesi de dahil) eklendi, izole worker (bwrap+cgroup+overlay) ilk kez gerçek makinede uçtan uca kanıtlandı — 2 gerçek bug (RLIMIT_NPROC per-UID yanlış hesap, tmpfs/bind mount sırası) bulunup düzeltildi; kalan tek madde (seccomp) artık "burada yapılamaz" değil, yalnız "henüz yapılmadı") | F2 + OS-isolated worker |
...
- [ ] OS izolasyonu: kullanıcı namespace/container/bubblewrap seçimi, ağ=kapalı varsayılanı, mount allowlist, no-new-privileges ve seccomp fizibilitesi. (16 Ağustos 2026, 10. tur düzeltmesi: seccomp burada gerçekten denenebilir — henüz yapılmadı, "burada yapılamaz" değil.)
