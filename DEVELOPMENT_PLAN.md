...
- [~] Metrikler (gizlilik-güvenli çekirdek) — başarı/verification oranı, policy kararları, capability dağılımı, uyarı sayısı; latency/token/CPU-RAM ölçümü hâlâ açık.
  ...
  - **Kalan:** latency/model-yükleme/token-üretimi süre ölçümleri, CPU/RAM/disk anlık kullanımı, ve bu özetin bir TUI komutuna (ör. `/metrics`) bağlanması.
...
- [~] Backup/retention komutları + restore tatbikatı (config/model/dataset rollback ve audit witness stratejisi hâlâ açık).
  ...
  - **Kalan:** config/model/dataset rollback (mevcut `/dataset`/`/model-runs` üstüne), audit export/witness stratejisi, ve bu yedek fonksiyonunun bir TUI komutuna bağlanması (şu an Runtime/persistence seviyesinde gerçek ve testli, kullanıcı arayüzünden çağrılmıyor).
- [~] Sürüm/migration yönetimi — compatibility check tamam (semantic version/changelog/config migration hâlâ açık).
  ...
  - **Kalan:** semantic version + changelog + gerçek config migration akışı.
