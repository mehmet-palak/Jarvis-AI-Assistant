...
- [x] MCP production hardening — ingress (protokol + sır redaksiyonu) + **egress + eklenti güven çekirdeği** ([ADR-0008](docs/adr/0008-mcp-architecture-and-security-layers.md), 22 Ağustos 2026). Çatı kural: "MCP taşımadır, yetki değil".
  ...
  - **Kanıt (JSON-RPC oturum boru hattı, `src/mcp_egress.rs`):** `McpEgressSession::call_tool` — dışarı filtresi→istek→ayrıştırma→içeri etiket; **düşmanca mock sunucu** ile 8 test (sır argümanı gönderilmeden durur, injection veri olarak sarılır, kaçırılan sır redakte edilir, JSON-RPC hatası tipli hata olur).
  - **Kalan (dürüst sınır, F7 deseni):** `McpRpcTransport` trait'inin gerçek gerçekleştirmesi — sunucuyu F4 sandbox'ında başlatıp canlı stdin/stdout JSON-RPC konuşan taşıma; F4/F7'nin "gerçek makinede doğrulanmalı" uyarısına tabi. Ağ taşımaları (uzak MCP) F10.
