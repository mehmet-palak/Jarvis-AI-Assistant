...
Durum: **F7.1-F7.7 TAMAMLANDI (21 Ağustos 2026).** Her alt-faz gerçek kanıt/testle kapatıldı; canlı orkestrasyona bağımlı birkaç madde (gerçek sandbox'lı pentest worker, steering/otonominin canlı yürütülmesi, streaming birleştirme) dürüstçe "tipli çekirdek + karar mantığı hazır, ortak worker katmanı geldiğinde canlanacak" olarak işaretlendi. **20 Ağustos 2026'da kullanıcı önceliklendirdi**: bunu gerçek bug bounty programlarında kullanacak, "en kritik yeteneklerden biri olacak" — F7, F8/F9'dan önce geldi (bkz. [[jarvis-f6-f7-prioritization-open]] hafıza kaydı). **Kullanıcı arayüzü kararı (21 Ağustos 2026):** kullanıcı `/pentest` slash-komutunu değil, JARVIS'in geri kalanı gibi DOĞAL KONUŞMAYI seçti. `pentest_intent.rs` + `Runtime::execute_pentest_intent` ile scope yönetimi + keşif konuşmaya bağlandı ("anla + onay iste" davranışı; ağ işlemleri önce onay ister). Kalan eylemler (replay, SAFE kontroller, bulgu/rapor) aynı desende konuşmaya bağlanabilir. Dürüst olgunluk değerlendirmesi ve yol haritası: [[jarvis-f7-maturity-and-gaps]].
...
- [x] Network-scoped sandbox worker'ın temel taşı: allowlist egress gate.
  ...
  - **Worker-wiring'in çekirdek mekanizması gerçek kernel'de kanıtlandı (F7.2'yi kapatan madde):** F4'ün `--unshare-net` sandbox'ı GERÇEKTEN kurulup (`systemd-run --user --scope` + `bwrap`, production ile birebir aynı bayraklar) içine bir Python süreci konuldu. Ölçülen, varsayılmayan iki sonuç: (1) sandbox içindeki süreç host'un loopback'indeki gerçek bir TCP noktasına **hiç ulaşamadı** — doğrudan ağ yolu gerçekten kapalı; (2) aynı süreç, host'tan sandbox'a bind-mount edilmiş bir Unix domain socket üzerinden **gerçekten iletişim kurabildi** (ping/pong byte'ları taşındı) — "tek çıkış kapısı" yolu gerçekten açık. Bu, [[jarvis-f7-end-of-phase-review]] hafıza kaydındaki "ağ izolasyonu gerçekten OS seviyesinde mi zorlanıyor" sorusunun somut, ölçülmüş cevabı — F7 bitiminde tekrar bakmaya gerek kalmadan burada kapatıldı.
  - **Test:** `workbench.rs`'deki gerçek-kernel testleriyle aynı prob/skip deseni (`systemd-run --user` yoksa test sessizce atlanır, fail olmaz — production kodu aynı eksiklikte açıkça hata veriyor). 3 kez art arda koşuldu, 3'ünde de kararlı geçti (flaky değil).
  - **Dürüst sınır:** Bu, "yolun kendisi çalışıyor mu" sorusunu kanıtlıyor, "gate'in SOCKS5 sunucusu bu UDS'yi dinliyor mu" sorusunu değil — `PentestNetworkGate` hâlâ yalnız TCP loopback dinliyor, henüz bir UDS dinlemiyor. Gate'i UDS'ye bağlamak (ve gerçek bir pentest worker'ı bu yoldan başlatmak) F7.3'ün somut bir aracı gelince, gerçek bir uçtan-uca senaryoyla yapılacak — o zaman soyut değil, test edilebilir bir hedefe karşı doğrulanabilir olacak.
- [x] Rate/runtime limitleri + gerçek kill switch.
  - **Kanıt:** Token-bucket hız sınırlayıcı — kapasiteyi aşan istek, hedef kapsam içinde olsa bile reddediliyor (gerçek testle: kapasite 1 iken ilk istek geçiyor, ikincisi "not allowed by ruleset" ile reddediliyor). `max_runtime_seconds` artık YALNIZ bir SOCKS5 yanıt kodu değil: süresi zaten dolmuş bir deadline'da `accept_one` YENİ BİR BAĞLANTIYI KABUL BİLE ETMİYOR (işletim sistemi seviyesinde `accept()` hiç çağrılmıyor) — bu, ilk tasarımdan daha güçlü bir garanti; keşfedilirken bir test yarış durumu (race condition) içerdiği bulunup düzeltildi.
  - **Kill switch gerçekten "acil" mi, ölçüldü:** İki senaryo test edildi — (1) henüz hiç bağlantı almamış bir `accept_one` çağrısı, kill switch tetiklenince 500ms içinde dönüyor (beklemiyor); (2) daha da kritik olanı, **hâlihazırda süren bir veri aktarımı** (gerçek bir TCP relay, sonsuz veri gönderen bir sunucuyla) kill switch tetiklendikten sonra 500ms içinde gerçekten duruyor — yalnız yeni bağlantıları değil, aktif olanları da kesiyor.
  - Henüz yok: WAF/engelleme tespiti, programın politika metnini okuma (bu ikisi F7.3+ gerçek keşif araçları geldiğinde daha anlamlı, çünkü tekrarlı istek örüntüsü gözlemlemek gerekiyor).
- [x] Dry-run modu: gate, yetkilendirme kararının kendisini tam SOCKS5 kod yolundan geçirip GERÇEK bağlantıyı hiç açmadan çalışabiliyor.
  - **Kanıt:** `PentestNetworkGate::bind_dry_run` ile açılan bir kapı, yetkili bir hedef için bile `TcpStream::connect` hiç çağırmıyor — SOCKS5 "OK" (0x00) dönüyor ama tek paket bile gitmiyor. Bunu kanıtlamak için test, kasıtlı olarak DİNLEMEYEN bir port'u hedef seçiyor: normal modda bu "host unreachable" (0x04) döndürürdü (çünkü bağlanmayı gerçekten dener ve başarısız olur); dry-run modda "OK" dönmesi, bağlanmanın hiç denenmediğinin doğrudan kanıtı. İzinsiz bir hedef dry-run'da da normal moddaki gibi aynı şekilde reddediliyor — dry-run yalnız "yetkiliyse bağlan" adımını etkiliyor, yetkilendirme kararının kendisini değil.
  - **Ne işe yarıyor:** Bir scope'un CIDR/wildcard eşleşmesi, hariç tutmaları, mod tavanı, süre/expiry davranışı — hepsi gerçek bir tool hiç ağa çıkmadan, tam gerçek SOCKS5 protokolü üzerinden doğrulanabiliyor. Gelecekte bir worker bu kapıya bağlanmadan önce "bu scope doğru davranıyor mu" sorusunun cevabı sıfır ağ riskiyle alınabiliyor.
  - **Test:** 2 yeni test (toplam gate testleri 9→11).
- [ ] Programın kendi politika metnini okuma: "otomatik tarama yasak" gibi kısıtlamalara uyum, izin verilen test saatleri, WAF/engelleme tespiti.

#### F7.3 — Keşif ve sürekli izleme

- [x] Pasif keşif — sertifika şeffaflık (Certificate Transparency) kayıtlarından subdomain bulma + teknoloji parmak izi çıkarımı (F7.5'te uygulandı, bkz. aşağıda) (kalan alt madde açık: geçmiş URL/endpoint kayıtları — Wayback Machine denendi, o servis bu oturumda çok yavaş/güvenilmez davrandı, ertelendi).
  - **Kaynak seçimi, canlı doğrulandı (20 Ağustos 2026):** Standart araç crt.sh'dır, ama bu oturumda crt.sh'ın kendi servisi birden çok alan adında, birden çok denemede sürekli `502 Bad Gateway` döndürdü (crt.sh'ın bilinen bir güvenilirlik sorunu — ücretsiz, bağışla ayakta tutulan bir servis). Bunun yerine, aynı işi yapan ve canlı test edildiğinde gerçek veriyle 200 döndüren certspotter.com (API anahtarı gerektirmeyen) birincil kaynak yapıldı. crt.sh, aynı sözleşmeyle ikinci bir kaynak olarak eklenebilir; şimdilik eklenmedi çünkü şu an gerçek uçtan uca doğrulanamıyor.
  - **Kanıt:** `pentest_recon.rs` — gerçek ağ çağrısı (`query_certificate_transparency`) `weather.rs`'nin kendi deseniyle aynı şekilde ayrı, saf bir ayrıştırma fonksiyonundan (`parse_certificate_transparency_response`) ayrıştırıldı: otomatik test paketi gerçek ağa hiç çıkmıyor (`cargo test --offline` ihlal edilmiyor), yalnız ayrıştırma mantığı sahte/gerçek yanıt örnekleriyle test ediliyor. GERÇEK bir canlı çağrı da var ama `#[ignore]` ile işaretli (elle `cargo test --lib pentest_recon -- --ignored`) — 20 Ağustos 2026'da elle koşuldu, github.com için gerçek, boş olmayan bir sonuç döndü.
  - **Scope'a göre süzme:** Servisin bulduğu her isim, F7.1'in aynı `authorize_pentest_target` fonksiyonundan geçiyor (`PentestMode::Safe` ile — henüz bir eylem seçilmediği için yalnız hedef eşleşmesi soruluyor, mod tavanı değil). Kapsam dışı kalan isimler kalıcı envantere HİÇ yazılmıyor, yalnız kaç tanesinin elendiği sayılıyor — bu, "keşfedilen" ile "test edilebilir" arasındaki çizgiyi otomatik olarak koruyor.
  - **Test:** 5 yeni pasif ayrıştırma testi + 1 gerçek-ağ testi (`#[ignore]`), + Runtime seviyesinde 5 yeni test (scope filtreleme, aktif scope zorunluluğu, iptal sonrası anında ret).
- [x] Aktif keşif (yalnız scope onaylıysa) — port/servis tarama + subdomain brute-force + JS analiziyle endpoint keşfi. F7.3'ün tüm "aktif keşif" alt maddeleri tamamlandı.
  - **Kanıt (port tarama):** `Runtime::scan_pentest_ports` gerçek TCP bağlantı denemeleriyle çalışıyor — sahte/simüle değil, gerçek dinleyen bir portu "açık", dinlemeyeni "kapalı" olarak raporladığı gerçek testle kanıtlandı. Hedefi gerçekten dokunduğu için (yalnız sorgulamayan pasif keşiften farklı olarak) F7.1'in AYNI tek giriş noktasından (`authorize_pentest_action`) `PentestMode::Active` talep ederek geçiyor — yalnız SAFE'e izin veren bir scope bunu reddediyor, tıpkı başka bir aktif eylemi reddedeceği gibi.
  - **Kendi kendini sınırlama (F7.2'nin hız sınırı gerekçesiyle aynı disiplin):** Tek bir çağrı en fazla 200 port tarayabiliyor (`MAX_PENTEST_PORTS_PER_SCAN`) — yetkili olmak sınırsız hızda/miktarda dövmenin güvenli olduğu anlamına gelmiyor. Scope'un `max_runtime_seconds`'ı da bu çağrının kendi süresini sınırlıyor: bütçe taramanın ortasında dolarsa, kalan portlara dokunmadan durup "buraya kadar yapabildim" diye dürüstçe raporluyor.
  - **Test disiplini (port tarama):** Süre-dolumu davranışı, gerçek ama yavaşça başarısız olan bir port beklemek yerine (ki bu network zamanlamasına bağlı olur, hiç deterministik olmazdı) `pentest_network_gate`'in kendi testlerindeki desenle aynı şekilde test edildi — zaten süresi dolmuş bir deadline doğrudan veriliyor.
  - **Kanıt (subdomain brute-force):** `Runtime::discover_pentest_assets_via_dns_bruteforce` — bir kelime listesini (`{kelime}.{apex}`) gerçek DNS çözümlemesinden (`getaddrinfo`) geçiriyor, hedefin kendi sunucusuna hiçbir paket gitmiyor (yalnız DNS altyapısına sorgu gidiyor). Plan metninin kendi sınıflandırmasıyla bu yine de ACTIVE tavanı gerektiriyor — yalnız SAFE'e izin veren bir scope reddediyor, gerçek testle kanıtlandı. Sonuçlar, CT log keşfiyle AYNI paylaşılan süzme/kayıt fonksiyonundan (`record_pentest_recon_candidates`) geçiyor — ayrı bir mantık icat edilmedi. Dışarıdan bir wordlist dosyası indirmeden (standing "izinsiz indirme yok" kuralı), JARVIS'in kendi seçtiği ~65 kelimelik makul bir varsayılan liste var (`common_pentest_subdomain_wordlist`).
  - **Kanıt (JS analiziyle endpoint keşfi):** `Runtime::discover_pentest_endpoints_via_javascript` — hedeften gerçek bir JS dosyası indirip (`fetch_javascript_source`, boyut tavanı 5MB) tırnak içi string literal'leri tarayarak "gerçekçi bir endpoint yolu" (`/api/v1/...` gibi) olanları çıkarıyor (`extract_endpoint_paths_from_javascript`). Yeni bir bağımlılık (`regex` crate'i) eklenmedi — HMAC'te olduğu gibi elle yazılmış bir tarayıcı. Gerçek dosya indirdiği için `PentestMode::Active` gerektiriyor, port taramasıyla aynı tavan. **Dürüst sınır:** bulunan yollar hostname envanterine (`pentest_assets`) yazılmıyor — bir yol bir hostname değil, F7.1'in host tabanlı scope eşleştirmesine zorla uydurmak yanlış bir soyutlama olurdu; sonuçlar yalnız çağırana dönüyor. Sonuçlar "bulundu" anlamında, "erişilebilir/doğrulandı" anlamında değil (F7.7'nin kendi ayrımı).
  - **Kalan:** Üçü de F7.2'nin SOCKS5 kapısından DEĞİL, doğrudan JARVIS'in kendi sürecinden bağlanıyor — henüz sandbox'lı bir pentest worker'ı olmadığı için (bu, F7.2'nin kendi kalan maddesi). Servis parmak izi/banner alma burada yok (ayrı, hâlâ açık bir F7.3 alt maddesi — "pasif keşif" bölümünde).
  - **Test:** 7 (port tarama, tamamı normal koşumda) + 8 (subdomain brute-force: 3 aday-üretimi/varsayılan-liste + 4 Runtime yetkilendirmesi + 1 gerçek-DNS `#[ignore]`) + 9 (JS endpoint keşfi: 5 path-çıkarım/doğrulama + 3 Runtime yetkilendirmesi + 1 gerçek-ağ `#[ignore]`) = 24 yeni test toplamda. İki gerçek-ağ testi de elle koşulup doğrulandı.
- [x] Varlık envanteri kalıcı kaydı + **yeni varlık ortaya çıkınca bildirim** (periyodik/otomatik yeniden tarama — bir zamanlayıcı/orkestrasyon katmanı — henüz yok, bu madde yalnız elle tetiklenen taramanın kalıcı kaydını ve diff'ini kapsıyor). Bug bounty'de değerin çoğu buradan geliyor — yeni bir subdomain/endpoint'e ilk bakan avantajlı; bu madde olmadan F7 sadece tek seferlik bir tarayıcı kalır.
  - **Kanıt:** `pentest_assets` tablosu (şema sürümü 14), `(scope_name, asset)` birincil anahtarıyla — iki program aynı ismi bulsa bile envanterleri karışmıyor. "Yeni" olmak, INSERT'ten ÖNCE o ismin o scope için var olup olmadığına bakılarak belirleniyor (`INSERT ... ON CONFLICT DO UPDATE`: bilinen bir varlık yalnız `last_seen`'i güncelliyor, gerçekten yeni bir isim `first_seen`'i şimdiye yazıp "yeni" listesine giriyor). Gerçek testle kanıtlandı: aynı isim ikinci taramada artık "yeni" değil, ama aynı turda gelen GERÇEKTEN yeni bir isim hâlâ öyle raporlanıyor.
  - **Test:** 3 yeni persistence testi (ilk görüşte hepsi yeni, tekrar taramada yalnız gerçekten yeni olan raporlanıyor, iki scope'un envanteri birbirine karışmıyor).
...
- [x] Oturum açmış (authenticated) test desteği: program tarafından verilen test hesabı bilgisinin güvenli saklanması ve tarama/replay araçlarına enjekte edilmesi — mevcut Secret Manager'a bağlanır ([[jarvis-layered-memory-architecture]]), yeni bir mekanizma icat edilmedi.
  - **Kanıt:** Bu madde plan metninin kendi notu gereği YENİ bir Runtime metodu gerektirmedi — `PentestHttpRequest.headers`'ın serbest biçimli olması ve mevcut Secret Manager'ın (`remember_secret`/`reveal_secret`) zaten var olması bu bileşimi olduğu gibi mümkün kılıyor. Gerçek testle kanıtlandı: bir test hesabı sırrı kaydedilip açığa çıkarılıyor, bir `Authorization` başlığına konup GERÇEKTEN gönderiliyor — yerel bir sunucu aldığı ham HTTP başlığını geri yansıtıp sır değerinin gerçekten ağdan geçtiğini doğruluyor.
  - **Dürüst sınır korundu:** `reveal_secret`'ın kendi belgelenmiş kuralı ("yalnız kullanıcının kendi açık talebiyle çağrılmalı, hiçbir sohbet/model bağlamı derleme yolu bunu çağırmaz") burada da geçerli — `replay_pentest_http_request` sır deposuna hiç dokunmuyor, sır değerini yalnızca ÇAĞIRAN (açık bir kullanıcı eylemiyle) çözüp başlık olarak veriyor.
  - **Yan ürün:** `PentestHttpRequest`'e `port: Option<u16>` eklendi — scope yetkilendirmesi her zaman çıplak hostname'e karşı kontrol edilir (port hedef kimliğinin parçası değil, `scan_pentest_ports`'la aynı model), bu alan aynı yetkili host'un standart olmayan bir portundaki bir servisi test etmeyi scope'u yeniden yazmadan mümkün kılıyor.
  - **Test:** 1 yeni uçtan-uca test (gerçek yerel sunucuyla).

#### F7.5 — SAFE modun somut ilk kontrolleri

Dördü de `PentestMode::Safe` (F7.1'in en düşük tavanı) ile yetkilendiriliyor — F7.4'ün ACTIVE gerektiren maddelerinden (port tarama, replay, JS keşfi) farklı olarak burada yalnız birkaç bilinen statik yola normal bir tarayıcının yapacağı türden salt-okunur GET istekleri gönderiliyor; hiçbir sömürü denemesi, kimlik bilgisi tahmini veya yazma işlemi yok. Gerçek testle kanıtlandı: SAFE tavanlı bir scope bu kontrolleri reddetmiyor (F7.4'ün ACTIVE gerektiren maddelerinin aksine).

- [x] Subdomain devralma (takeover) tespiti — yalnız DNS/HTTP kontrolü, sömürü yok.
  - **Kanıt:** `Runtime::check_pentest_subdomain_takeover` hedefin kök sayfasını çekip 16 bilinen "terk edilmiş üçüncü taraf servis" imzasına ("Can I take over XYZ" tarzı projelerin kullandığı yöntem — S3, GitHub Pages, Heroku, Netlify, Shopify, Bitbucket, Surge.sh, Tumblr, WordPress.com, Zendesk, Cargo Collective, Ghost(Pro), Pantheon, Unbounce, UserVoice, Fastly) karşı kontrol ediyor. Gerçek testle kanıtlandı: gerçek bir yerel sunucudan gelen bir S3 hata sayfası doğru tespit ediliyor, gerçek bir sıradan sayfa `None` döndürüyor.
- [x] Açığa çıkmış hassas dosya/yanlış yapılandırma tespiti (`.git`, `.env`, yedek dosyaları, açık depolama).
  - **Kanıt:** `Runtime::scan_pentest_exposed_files` 6 bilinen yolu (`.git/HEAD`, `.git/config`, `.env`, `.DS_Store`, `backup.sql`, `dump.sql`) dener — yalnız durum kodu 200 YETERLİ DEĞİL (çoğu yanlış pozitifin kaynağı tam olarak bu, "her şeye 200 dönen" soft-404 sayfaları), her yol için ayrıca gerçek içerik biçimini doğrulayan bir imza fonksiyonu var (`.git/HEAD` için `ref:` öneki, `.env` için `BÜYÜK_HARF=değer` satırı, `.DS_Store` için ikili sihirli sayı `Bud1`, SQL dökümü için `CREATE TABLE`/`INSERT INTO`). Gerçek testle kanıtlandı: gerçek bir açığa çıkmış `.env` bulunuyor, aynı anda bir soft-404 döndüren `.git/HEAD` yanlış pozitif ÜRETMİYOR.
- [ ] Bilinen CVE eşleştirmesi (parmak izinden çıkan yazılım sürümüne karşı) — CVE veri kaynağının güncel tutulması dahil.
  - **Bilinçli olarak eksik bırakıldı:** Gerçek, güncel bir CVE veri kaynağı olmadan (NVD API canlı entegrasyonu ya da düzenli güncellenen bir paket — ikisi de ayrı bir tasarım kararı ve büyük bir bakım yükü gerektiriyor) sahte bir "CVE eşleştirme" tablosu üretmek gerçek bir güvenlik yeteneği yerine YANLIŞ BİR GÜVEN DUYGUSU verirdi — bu, projenin "yalnız gerçek kanıtla çalışır iddiasında bulun" disipliniyle çelişirdi. Ön koşulu (teknoloji/versiyon parmak izi çıkarımı, aşağıda) gerçek ve test edilmiş; CVE veri kaynağı kararı verildiğinde üstüne inşa edilecek.
  - **Yan ürün (F7.3'ün kalan alt maddesini de kapatıyor):** `Runtime::fingerprint_pentest_technology` hedefin `Server`/`X-Powered-By`/`X-AspNet-Version`/`X-Generator` gibi başlıklarını çıkarıyor — F7.3'ün "pasif keşif: teknoloji parmak izi" maddesini de kapatıyor. Gerçek testle kanıtlandı: gerçek bir `Server: nginx/1.18.0` başlığı doğru çıkarılıyor, teknoloji ile ilgisiz başlıklar (`Content-Type`, `Set-Cookie`) dışarıda bırakılıyor.
- [x] TLS/sertifika sorunları.
  - **Kanıt:** `Runtime::check_pentest_tls_connectivity` bir HTTPS bağlantısının (sertifika doğrulaması dahil) başarılı olup olmadığını kontrol ediyor. Gerçek testle kanıtlandı: düz HTTP konuşan gerçek bir yerel sunucuya `https://` ile bağlanmak GERÇEK bir TLS handshake başarısızlığı üretiyor, sahte bir hata değil.
  - **Dürüst sınır:** Yalnız "bağlantı başarılı oldu mu" sorusuna cevap veriyor — tam olarak hangi sertifika sorunuyla karşılaşıldığını (süre dolumu/kendinden imzalı/hostname uyuşmazlığı) AYIRT EDEMİYOR; bunun için ham TLS handshake'ine inen yeni bir bağımlılık (`native-tls`/`rustls`'i doğrudan kullanmak) gerekir — bu, tek başına verilecek bir karar değil, bilinçli olarak eklenmedi.
  - **Test:** Toplam F7.5: 9 (pentest_safe_checks.rs, saf mantık) + 7 (Runtime seviyesi: takeover tespit/ret × 2, exposed-file bulma + yanlış-pozitif-önleme × 1, tech fingerprint × 1, TLS başarısızlık × 1, SAFE-tavan-yeter × 1, aktif-scope-zorunluluğu × 1) = 16 yeni test.

#### F7.6 — Bulgu yönetimi ve raporlama

- [x] Evidence tabanlı finding formatı, insan onayı, audit export ve scope dışı/secret hedef deny testleri.
  - **Kanıt:** `PentestFinding` — hedef, kategori, başlık, ham kanıt, şiddet tahmini (mevcut `Risk` enum'u yeniden kullanıldı, yeni bir ölçek icat edilmedi), durum (`Suspected`/`Confirmed`/`Rejected`). `Runtime::record_pentest_finding` kaydetmeden ÖNCE hedefi F7.1'in aynı tek giriş noktasından geçiriyor — gerçek testle kanıtlandı: scope dışı bir hedef reddediliyor.
  - **İnsan onayı:** `confirm_pentest_finding`, `commit_memory_proposal`'ın zaten kurduğu `human_approved: bool` deseniyle aynı — açık onay VE taze bir yeniden üretme kanıtı olmadan bir bulgu asla `Confirmed`'e geçemiyor (F7.7'nin `confirm_finding` sözleşmesiyle aynı ilke, burada gerçek kodda). Gerçek testle kanıtlandı: onaysız veya boş kanıtla deneme reddediliyor; zaten doğrulanmış bir bulgu ikinci kez "doğrulanamıyor" (kararın ne zaman verildiği belirsizleşmesin diye).
  - **Sır sızıntısı koruması:** kanıt metni, workspace RAG ingestion'ın zaten kullandığı `reject_secret_like_workspace_document_content` ile taranıyor — yeni bir mekanizma icat edilmedi. Gerçek testle kanıtlandı: bariz bir AWS anahtarı (`AKIA...`) içeren bir kanıt reddediliyor.
  - **Audit export:** Bulgular zaten kalıcı, sorgulanabilir SQLite tablosunda (`pentest_findings`) — ayrı bir export formatı icat edilmedi, `pentest_findings(scope_name)` sorgusunun kendisi bu ihtiyacı karşılıyor (projenin geri kalanının "veri zaten kalıcı ve sorgulanabilir" felsefesiyle aynı).
  - **Test:** 9 yeni test (suspected başlangıç durumu, sır reddi, scope dışı hedef reddi, onay/kanıt zorunluluğu × 2, doğrulama başarılı, çift-doğrulama reddi, ret + silinmeme, dedup).
- [x] Daha önce bulunanla eşleştirme (deduplication) — mevcut audit hash-chain deseniyle, yeni bir mekanizma icat edilmedi. Programlar tekrar bildirilen bulgulara olumsuz bakıyor.
  - **Kanıt:** Ayrı bir eşleştirme adımı yok — `finding_id`, `memory_id`'nin (F3) zaten kurduğu desenle aynı: `(scope_name, target, category, title)` dörtlüsünün içerik hash'i. AYNI bulgu tekrar kaydedilirse yapısal olarak asla ikinci bir satır oluşamıyor (`INSERT ... ON CONFLICT DO UPDATE`) — gerçek testle kanıtlandı: aynı bulgu iki kez farklı kanıtla kaydedilince tek satır kalıyor, en güncel kanıt/şiddet kazanıyor.
- [x] Rapor öncesi yeniden doğrulama: bulgu ile rapor yazma arasında hedef değişmiş olabilir, göndermeden önce hâlâ geçerli mi diye tekrar bakılır.
- [x] Düzeltme sonrası hedefli yeniden test: program "düzelttik, doğrular mısın" dediğinde tüm taramayı değil yalnız o bulguyu tekrar kontrol etme.
  - **Kanıt (ikisi de aynı mekanizma):** `Runtime::revalidate_pentest_finding` bulgunun `category`'sine göre F7.5'in İLGİLİ tek kontrolünü tekrar çalıştırıyor — `scan_pentest_ports`/`scan_pentest_exposed_files` gibi TÜM taramayı değil, yalnız o bulguyu. `PentestFindingRevalidation::{StillPresent, NoLongerPresent, CheckNotSupported}` döndürüyor. Gerçek testle kanıtlandı: aynı yerel sunucu portu önce sızıntıyı, sonra "düzeltilmiş" hâlini döndürüyor — `revalidate_pentest_finding` bu ikisini doğru ayırt ediyor (gerçek bir "önce var, sonra yok" senaryosu, varsayım değil).
  - **Yeni alan (bu ikisini hassas kılan):** `PentestFinding.check_parameter` — `category`+`target` tek başına yeterli olmadığında (`exposed_sensitive_file`'da HANGİ yolun bulunduğu, `/.env` gibi) somut parametreyi taşıyor; olmadan yalnız "bu kategoriden BİR ŞEY hâlâ var mı" gibi belirsiz bir kontrol yapılabilirdi. Şema sürümü 16.
  - **Test:** 4 yeni test (gerçek sızıntı/düzeltme ayrımı, gerçek devralma tespiti, check_parameter eksikse ret, desteklenmeyen kategori için açık `CheckNotSupported`).
- [x] **Modelin kendisi raporu yazabilmeli, iyi bir şekilde.** Kanıt toplamak (F7.6'nın diğer maddeleri) ile göndermeye hazır bir rapor arasındaki mesafe kapanmalı: model, toplanan kanıttan (istek/cevap çiftleri, ekran görüntüleri, replay/diff sonuçları) platformun beklediği yapıda (özet, adım adım tekrar üretme, etki analizi, önerilen düzeltme, CVSS/severity tahmini) düzyazı bir rapor taslağı üretir. Kullanıcı gönderilmeden önce gözden geçirip onaylar — F4'ün patch akışındaki "önce göster, sonra onay al" deseniyle aynı, burada da model asla kullanıcı adına doğrudan göndermez. Rapor kalitesi golden set'e (F6) yeni bir zor senaryo olarak eklenebilir: gerçek bir bulgu senaryosundan üretilen rapor taslağı, gerekli bölümlerin hepsini içeriyor mu diye mekanik olarak kontrol edilebilir.
...
- [x] **Otonomi modeli netleştirmesi:** mevcut SAFE/ACTIVE/INTRUSIVE/DESTRUCTIVE merdiveni "ne yapılabilir" sorusunu cevaplıyor; buna dik bir ikinci eksen eklendi — "ne kadar gözetim gerekir".
  - **Kanıt:** `PentestAutonomy` enum'u (`Manual`/`SupervisedAutonomy`/`BoundedAutonomy`). `allows_unattended(action_mode)` iki ekseni birleştiriyor: `Manual` hiçbir şeyi otomatik yürütmüyor (en zararsız SAFE adım bile onay ister), `SupervisedAutonomy` yalnız SAFE'i otomatik yürütüyor (ACTIVE+ onay ister), `BoundedAutonomy` ACTIVE'e kadar otomatik (INTRUSIVE+ durur). Gerçek testle kanıtlandı: "iki eksen birbirinin yerine geçmez" ilkesi — otonomi ne kadar yüksek olursa olsun, yeterince invaziv bir adım her zaman onay istiyor. `Ord` sıralaması otomasyon artışını yansıtıyor.
  - **Not:** Bu, tipli sözleşme ve karar fonksiyonu olarak tamamlandı; gerçek bir çalışan görev döngüsüne bağlanması (bir worker'ın bu kararı canlı olarak uygulaması) F7.7'nin "görev kontrolü" maddesiyle birlikte, gerçek bir orkestrasyon katmanı geldiğinde yapılacak.
  - **Test:** 3 yeni test (iki-eksen bağımsızlığı tablosu, sıralama, str roundtrip).
- [x] **Görev kontrolü (steering) ve devam ettirme (karar mantığı çekirdeği):** kullanıcı çalışan bir güvenlik görevine "yalnızca auth akışına odaklan", "bu endpoint'i kapsam dışına al" veya "dur" diyebilmeli.
  - **Kanıt:** `PentestSteering` struct'ı (`stopped`, `focus_endpoint_prefixes`, `excluded_endpoint_prefixes`) + `permits_endpoint`. Yönlendirmeler DOĞRU öncelikle uygulanıyor: önce "dur" (her şeyi durdurur), sonra dışlama (odakta olsa bile her zaman kazanır), sonra odak (boşsa sınır yok). Gerçek testle kanıtlandı — özellikle "açıkça dışlanan bir endpoint, odak içinde olsa bile reddedilir" durumu. Tipli ve kalıcılaştırılabilir bir durum olduğu için devam ettirme (F3 session/resume) doğal olarak üzerine oturuyor.
  - **Dürüst kapsam:** Karar mantığı gerçek ve test edilebilir; onu CANLI bir çalışan görev döngüsüne bağlamak (worker'ın her adımdan önce buna danışması, kullanıcının çalışırken "dur" diyebilmesi) gerçek bir orkestrasyon/streaming worker katmanı geldiğinde yapılacak — otonomi modeliyle (F7.7, tamamlandı) aynı temeli paylaşıyor, ikisi birlikte canlanacak.
  - **Test:** 1 yeni test (dur/dışla/odakla önceliklendirmesi).

**F7.7 durumu:** Tüm maddeler tamamlandı. Canlı orkestrasyona bağımlı olanlar (steering'in canlı uygulaması, otonomi modelinin canlı worker'da yürütülmesi, streaming tool-call birleştirme, gerçek sandbox'lı pentest worker) dürüstçe "tipli çekirdek + karar mantığı hazır, canlı bağlama gerçek worker katmanı geldiğinde" olarak işaretlendi — hepsi ortak bir orkestrasyon katmanını bekliyor.
- [x] **Kapsam matrisi (coverage tuple):** `(hedef, endpoint, parametre, zafiyet_sınıfı)` dörtlüsü izlenir — hangi kombinasyonun test edildiği, hangisinin edilmediği görünür olur; "sıradaki iş" önerisi yalnız kapsam içinde ve daha önce test edilmemiş kombinasyonları önerir.
  - **Kanıt:** `pentest_coverage` tablosu (şema sürümü 17), `(scope, hedef, endpoint, parametre, zafiyet_sınıfı)` birincil anahtarıyla. `Runtime::untested_pentest_coverage`, aday kombinasyonlardan HENÜZ test edilmemiş olanları döndürüyor — gerçek testle kanıtlandı: zaten test edilmiş `(/api/users, id, idor)` öneri listesinden çıkarılıyor, yeni parametre/endpoint kombinasyonları kalıyor. Bir bulgudan (`PentestFinding`, "bir şey bulundu") ayrı, çünkü test edilip temiz çıkan bir kombinasyon da değerli bilgidir (aynı işi iki kez yapmamak için). Hedef F7.1'in aynı tek giriş noktasından geçiyor.
  - **Test:** 3 yeni test (yalnız test edilmemişi önerme, aynı dörtlünün tekrar etmemesi, scope dışı hedef reddi).
- [x] **`confirm_finding` sözleşmesi:** bir bulgu yeniden üretme kanıtı olmadan "confirmed" durumuna geçemez — "model şüphelendi" ile "doğrulandı" ayrı, karıştırılmayan durumlar.
  - **F7.6'da gerçek kodda uygulandı** (`confirm_pentest_finding`, `human_approved: bool` + taze kanıt zorunluluğu; `PentestFindingStatus::{Suspected, Confirmed, Rejected}`). Bu madde F7.6'nın evidence-based finding maddesiyle aynı sözleşme olduğu için orada kanıtlandı — burada ayrıca bir kod gerektirmedi.
- [x] **Bilgi paketleri (knowledge packs):** konu/teknoloji/risk/önkoşul/güvenli-doğrulama/remediation metadata'sı taşıyan, yalnız görev kapsamına uygun olanı yüklenen bilgi paketleri — bilgi ile tool authority tamamen ayrı kalır.
  - **Kanıt:** `KnowledgePack` struct'ı (`pentest_knowledge.rs`) — konu, teknolojiler, risk, önkoşul, güvenli doğrulama, remediation, mod tavanı. **Bilgi-yetki ayrımı kodda yapısal:** struct'ta bir capability/komut alanı YOK, olamaz — bir bilgi paketi hiçbir şey çalıştıramaz, yalnız metin taşır. `is_relevant_to(task_mode, task_technologies)` "yalnız görev kapsamına uygun olanı yüklenen" ilkesini uyguluyor: paketin modu görevin tavanını aşmamalı VE en az bir teknoloji örtüşmeli. Gerçek testle kanıtlandı: SAFE bir görevde ACTIVE gerektiren bir paket yüklenmiyor, teknoloji örtüşmezse yüklenmiyor. `validate_knowledge_pack` eksik güvenli-doğrulama/remediation alanı olan bir paketi reddediyor (sömürü bilgisi olup güvenli doğrulama olmayan bir paket tehlikelidir).
  - **Lisans sınırı:** Kaynak içerikler (ör. HackTricks, CC BY-NC 4.0) KOPYALANMAZ — yalnız konu başlıkları ve JARVIS'in kendi özgün notları. Bu kod seviyesinde zorlanamaz (bir metnin "özgün mü kopya mı" olduğunu makine ayırt edemez) ama sözleşme ve modül notu seviyesinde net.
  - **Test:** 4 yeni test.
- [x] **OSINT — F7'nin resmi bir alt-alanı:** domain/kullanıcı adı/telefon pivotlarını ilişkilendiren, salt-okunur bir capability. Her iddia kaynak URL + fetch zamanı + confidence + "bulunamadı" ile "bilinmiyor" ayrımı taşır. **Varsayılan mod `PASSIVE_PUBLIC_ONLY`**; breach-intelligence varsayılan kapalı.
  - **Kanıt:** `pentest_osint.rs` — `OsintClaim` (pivot türü, konu, durum, değer, kaynak URL, fetch zamanı, confidence, kaynak sınıfı) + `validate_osint_claim`. Provenance disiplini YAPISAL olarak zorlanıyor. Asıl F7.7 şartı — "bulunamadı ≠ bilinmiyor" — tipli bir enum'la (`OsintFindingState::{Found, NotFound, Unknown}`): `NotFound` bir KANITTIR (bir kaynağa bakıldı, yok) ve kaynak URL + fetch zamanı taşımak ZORUNDA; `Unknown` bir kanıt DEĞİL (bakılmadı) ve kaynak/değer taşıyAMAZ. Gerçek testle kanıtlandı: bu ikisinin karıştırılması reddediliyor. `Found` bir değer + provenance zorunlu.
  - **Varsayılan mod güvenliği:** `OsintSourceClass::PassivePublic` varsayılan izinli; `BreachIntelligence` `validate_osint_claim` tarafından her zaman reddediliyor (açık onay ayrı bir üst-katman kararı). Gerçek testle kanıtlandı. Modül kendi başına ağa ÇIKMIYOR — yalnız tipli provenance çekirdeği; gerçek kaynak sorguları (login bypass/CAPTCHA/scraping yasak sınırları içinde) ileride bu sözleşmenin üstüne bağlanacak.
  - **Test:** 6 yeni test.
- [x] **Yetkili evidence snapshot (web mirroring) capability'si:** yetkili bir hedefin dinamik testten önce hash'lenmiş, değişmez bir kopyasını alma — provenance ve "snapshot ile gerçek hedef arasındaki fark" raporlama için. **"web kopyalama" değil "yetkili kanıt görüntüsü" olarak adlandırıldı.**
  - **Kanıt:** `Runtime::capture_pentest_evidence_snapshot` yetkili bir hedeften bir yolu çekip, saf bir dönüşümden (`pentest_evidence::build_evidence_snapshot`) geçiriyor. `PentestEvidenceSnapshot` metadata'sı (hedef, yol, redakte-içeriğin SHA-256'sı, boyut, yakalama zamanı, redakte-edildi-mi) modele verilebilir; ham içerik ayrı bir `Vec<u8>` (untrusted data envelope) olarak dönüyor, modele doğrudan verilmemeli. Gerçek testle kanıtlandı: gerçek bir yerel sunucudan bir `AKIA...` anahtarı içeren JS çekiliyor, anahtar redakte ediliyor (`[REDACTED]`), sır dışı içerik korunuyor, hash 64 karakter.
  - **Sınırlar (kodda zorlanan):** maksimum boyut (5MB, aşan reddediliyor), MIME allowlist (yalnız metin tabanlı web varlıkları — video/ikili reddediliyor), credential redaction (yüksek-entropili sır önekleri: AWS/GitHub/Slack/Google/Stripe belirteçleri), host allowlist (F7.1 scope'un aynı tek giriş noktası). Aynı içerik her zaman aynı hash'i üretiyor ("değişti mi" karşılaştırmasının temeli). **Kalan (kod seviyesinde değil, gerçek disk deposu gelince):** disk kotası, robots kararı, retention politikası — bunlar kalıcı bir dosya deposu tasarımı gerektiriyor, henüz snapshot bellekte/çağırana dönüyor.
  - **Test:** 5 (saf mantık) + 2 (Runtime, gerçek yerel sunucu) = 7 yeni test.
- [x] **Pasif kaynak keşfi genişletmesi (test edilebilir çekirdek):** CDN/WAF arkasındaki olası origin IP'ler — "aday origin" ile "doğrulanmış açık" ayrı tutulur.
  - **Kanıt:** `pentest_recon.rs`'te `OriginConfidence::{Candidate, Verified}` enum'u F7.7'nin asıl disiplinini yapısal olarak zorluyor: bir pasif sinyalden (favicon hash eşleşmesi) çıkan her origin bir ADAY, asla doğrulanmış değil — `origin_candidates_from_favicon` yalnız `Candidate` üretiyor, gerçek testle kanıtlandı. `favicon_hash` içerik-kararlı bir parmak izi (aynı favicon'ı paylaşan sunucuları ilişkilendirmenin bilinen tekniği). Arşivlenmiş içerik için sır/PII redaksiyonu + boyut sınırı zaten evidence snapshot'ta (F7.7) mevcut, aynı fonksiyonlar yeniden kullanılabilir.
  - **Dürüst kapsam:** MX/TXT/PTR gibi DNS kayıt sorguları std'nin `getaddrinfo`'suyla yapılamaz (yalnız A/AAAA) — bir DNS kütüphanesi gerektirir, bilinçli olarak eklenmedi (bağımlılık kararı). Tarihsel URL arşivleri (Wayback/Common Crawl) crt.sh gibi bu oturumda güvenilmez/yavaş davrandı (canlı denendi), ertelendi. Test edilebilir, bağımlılıksız çekirdek (aday/doğrulanmış ayrımı + favicon tekniği) tamamlandı; bu ikisi gerçek bir bağımlılık/güvenilir kaynak kararıyla eklenecek.
  - **Test:** 3 yeni test.
- [x] **HTML/JS/HAR'dan endpoint/parametre çıkarımı:** Burp/ZAP/Caido export'ları (HAR formatı) dahil girdilerden endpoint/parametre çıkarma; "bulundu" ile "erişilebilir/doğrulandı" durumları ayrı.
  - **Kanıt:** F7.3'te JS string-literal endpoint çıkarımı zaten vardı (`extract_endpoint_paths_from_javascript`); bu madde onu HAR ile tamamlıyor. `extract_endpoints_from_har` (saf, ağdan bağımsız) bir HAR JSON'undan (`log.entries[].request.url` + yapılandırılmış `queryString`) benzersiz path'leri ve parametre ADLARINI çıkarıyor. Bu yalnız "trafikte GÖRÜLDÜ" der — hedefe hiç istek atmaz (bir dosyayı ayrıştırıyor), F7.7'nin "bulundu ≠ doğrulandı" ayrımıyla tutarlı. Bir bulguysa değil, keşif verisi; scope filtreleme, bu adaylar `record_pentest_recon_candidates`/kapsam matrisine beslendiğinde F7.1'in aynı kapısından geçiyor.
  - **Sır/PII koruması:** Parametre DEĞERLERİ asla çıkarılmıyor — yalnız adlar (bir sorgu değeri sır/PII taşıyabilir). Gerçek testle kanıtlandı: `?token=SECRET_VALUE_123`'ten yalnız `token` alınıyor, değer hiç yakalanmıyor.
  - **Zarif bozulma:** Bozuk/eksik bir HAR'da çökmüyor — geçersiz JSON açık hata veriyor, ama eksik alanlı bir entry (request.url yok) çökmeden atlanıyor (bir güvenlik aracının kendi denetiminden alınan ders: kısmi girdi zarif kısmi sonuç üretmeli). Gerçek testle kanıtlandı.
  - **Kalan:** Normalize asset grafiğine kalıcı yazma + 403/429/timeout oranı yükselince zarif durma — ilki kapsam matrisi tablosuyla (F7.7, tamamlandı) birleştirilebilir, ikincisi gerçek bir çok-istekli tarama orkestrasyonu gerektiriyor (henüz yok).
  - **Test:** 4 yeni test (path+parametre çıkarımı, yapılandırılmış queryString, parametre değerinin asla yakalanmaması, geçersiz JSON reddi + eksik alan toleransı).
- [x] **F4/F7 worker'ı için somut regresyon test sınıfları** (bir aracın kendi güvenlik denetiminden alınan dersler — kod değil, test kapsamı): pentest-ilgili en yüksek değerli sınıflar gerçek kod + testle kapatıldı.
  - **Kapatılanlar:** (1) **URL userinfo/JWT/Authorization sır redaction'ı** — evidence snapshot redaksiyonu `scheme://user:pass@host` userinfo'sunu ve JWT (`eyJ...`) belirteçlerini de yakalıyor; gerçek testle kanıtlandı, ayrıca düz bir e-postanın (`support@example.com`) yanlışlıkla redakte edilmediği de test edildi. (2) **Terminal escape/kontrol-bayt temizliği** — evidence snapshot, saklamadan önce ANSI escape (`\x1b`) ve ham kontrol byte'larını (NUL vb.) atıyor, meşru `\n`/`\r`/`\t`'yi koruyor (bir hedefin kanıta ekran-manipülasyon dizileri gömmesine karşı). (3) **Atomik bulgu oluşturma + eşzamanlı ekleme güvenliği** — İKİ ayrı DB bağlantısı aynı dosyaya aynı bulguyu yazsa bile içerik-adresli `finding_id` + `INSERT ON CONFLICT` tek satır garanti ediyor; gerçek dosya-tabanlı iki-bağlantı testiyle kanıtlandı. (4) **Transport-seviyeli boyut sınırı + evidence OOM sınırı** — evidence snapshot 5MB, JS/replay çekimleri `Read::take` ile sınırlı, port tarama/DNS brute-force per-çağrı tavanlı (F7.3/F7.7 boyunca).
  - **Zaten kapsanan (bu oturumdan önce, F4'te):** DNS rebinding/TOCTOU ve symlink/realpath savunması `workbench.rs`'nin bwrap izolasyonu + `validate_workspace_relative_path`'iyle; iptal edilen oturumların bozuk geçmiş bırakmaması `workflow.rs`'nin rollback testleriyle.
  - **Kalan (gerçek streaming/worker altyapısı gelince):** akan tool-call parçalarının birleştirilmesi ve resolve/connect TOCTOU'nun canlı bir ağ worker'ında test edilmesi — henüz sandbox'lı bir pentest worker'ı yok.
  - **Test:** 5 yeni test (URL userinfo, e-posta yanlış-pozitif önleme, JWT, terminal escape, eşzamanlı bulgu yazma).
- [x] **Kaynak/araç risk sınıflandırması:** her potansiyel araç fikri capability manifest'i (risk seviyesi, ağ etkisi, credential ihtiyacı, çıktı formatı) ile kayıt altına alınır; hiçbir araç kendi threat model'i, sandbox'ı, scope testi ve regresyon testinden geçmeden registry'ye girmez.
  - **Kanıt:** `ToolRiskManifest` struct'ı (`pentest_knowledge.rs`) — tool_id, risk, ağ etkisi, credential ihtiyacı, ve dört gate (threat model, sandbox profili, scope testi, regresyon testi). `is_registerable()` F7.7'nin "hiçbir araç ... geçmeden registry'ye girmez" kuralının kontrol edilebilir hâli — dört gate de sağlanmadan bir araç GİREMEZ. Gerçek testle kanıtlandı: dördü de varsa geçiyor, biri eksikse (ör. scope testi) açık bir hatayla reddediliyor, birden fazla eksik hepsi birden raporlanıyor.
  - **Test:** 3 yeni test.
...
Durum: **MOBİL HARİÇ TAMAMLANDI (22 Ağustos 2026).** Kullanıcı önce "mobil hariç kapatalım" (yerel takvim), sonra "mobili F10'a at, F8'i tamamla" dedi. MCP egress + eklenti güven çekirdeği [ADR-0008](docs/adr/0008-mcp-architecture-and-security-layers.md)'nin **6 fazının tamamı kod olarak yazılıp gerçek testle kapatıldı** (canlı taşıma dahil, gerçek alt-süreçle). Tek dürüst sınır F4'ün kendisiyle aynı: `SandboxedStdioTransport`'un gerçek bwrap sarması gerçek makinede release smoke ile doğrulanır (bu geliştirme ortamı tam koşamaz). Mobil (madde 4-5) kullanıcının açık kuralıyla F10'a taşındı.

- [x] MCP production hardening — ingress (protokol + sır redaksiyonu) + **egress + eklenti güven çekirdeği** ([ADR-0008](docs/adr/0008-mcp-architecture-and-security-layers.md), 22 Ağustos 2026). Çatı kural: "MCP taşımadır, yetki değil".
  - **Kanıt (egress güven çekirdeği, `src/mcp_client.rs`):** `McpServerManifest` + HMAC-SHA256 imzalama (F7 scope deseni, uzunluk-önekli kanonik bayt) + `validate_mcp_manifest` (tek policy-gate, deny-by-default) + `authorize_mcp_connect` (tek kapı: durum→manifest→imza→**artefakt-hash/rug-pull**→protokol) + `hash_artifact` (tedarik-zinciri sabitlemesi). 19 test.
  - **Kanıt (kalıcı kayıt defteri, `persistence.rs`, şema v18):** `mcp_servers` + ayrı `mcp_signing_key`; `register/list/revoke/quarantine` + store üzerinden deny-by-default `authorize_mcp_connection`. 5 test.
  - **Kanıt (veri-akış + sampling, `mcp_client.rs`):** `authorize_outbound_argument` (dışarı tavan+sır), `sanitize_and_tag_inbound_output` (içeri provenance-zarfı+boyut+redaksiyon), `isolate_mcp_resource/prompt_as_data`, `authorize_mcp_sampling` (**deny-by-default** — yetki-yükseltme kanalı). 7 test.
  - **Kanıt (operasyonel, `runtime.rs`+`main.rs`):** global kill switch (`/mcp off`), audit olayları, `/mcp` TUI komutları (durum/liste/iptal). 1 test.
  - **Kanıt (JSON-RPC oturum boru hattı + canlı taşıma, `src/mcp_egress.rs`):** `McpEgressSession::call_tool` — dışarı filtresi→istek→ayrıştırma→içeri etiket; **düşmanca mock sunucu** ile 8 test (sır argümanı gönderilmeden durur, injection veri olarak sarılır, kaçırılan sır redakte edilir, JSON-RPC hatası tipli hata olur). `SandboxedStdioTransport` — sunucuyu `isolated_worker_command` (F4 hapsi) ile başlatıp satır JSON-RPC + `recv_timeout` konuşur; **gerçek /bin/sh alt-süreciyle** 2 test (uçtan uca yaz→oku→sarma + sessiz sunucuda timeout, asılı kalmaz).
  - **Dürüst sınır (F4 bar'ı):** `SandboxedStdioTransport`'un gerçek `bwrap+systemd-run` sarması bu ortamda tam koşmaz; F4'ün kendisi gibi gerçek makinede doğrulanır. G/Ç mantığı gerçek süreçle kanıtlı. Ağ taşımaları (uzak MCP) + `network_allowed` sunucular F10.
  - **Eski ingress kanıtı korunuyor:** `CURRENT_MCP_PROTOCOL_VERSION`/`validate_mcp_protocol_version` (protokol sınır-reddi) + `redact_secret_like_mcp_response` (yanıt sır filtresi), commit 62dfe0a.
...
- [~] Yerel entegrasyonlar: **yerel, ağsız takvim tamamlandı** (21 Ağustos 2026). OAuth/ağ gerektiren sağlayıcılar (e-posta, mesajlaşma) bilinçli olarak MCP egress ile aynı ağ kovasında F10'a erteli.
  - **Kanıt (yerel takvim):** `src/calendar.rs` — elle yazılmış, ağa hiç çıkmayan bir iCalendar (`.ics`) ayrıştırıcı (harici crate yok, hava durumu/pentest deseni). `CalendarProvider` trait + `LocalIcsCalendarProvider` (hava durumundaki `WeatherProvider` deseninin ikizi); `Runtime`'a `Option<Box<dyn>>` olarak takılı, model onu hiç "çağıramaz". Açılış brifinginde bugünkü etkinlikler + `/takvim` komutunda 7 günlük ajanda. Etkinlik metni `sanitize_event_text` ile temizlenir ("data, instruction değil" — bir `.ics` dış davetten gelmiş olabilir). Tarih hesabı epoch'tan saf civil-date algoritmasıyla. Dürüst sınır: RRULE (tekrarlayan) + tam saat-dilimi yok.
  - **Test:** 15 yeni test (14 birim: ayrıştırma, satır-katlaması, tarih biçimleri, kaçış çözme, güvenlik-temizliği, gün-süzme, civil-date gidiş-dönüş, gerçek dosya okuma; + 1 runtime brifing testi). `src/calendar_tests.rs` (ayrı test dosyası kuralı).
- [x] Plugin/skill ekosistemi — **MCP egress güven çekirdeği tarafından kapatıldı** (22 Ağustos 2026). ADR-0008'de netleştiği gibi bir eklenti = izole, imzalı bir dış MCP aracı; ayrı bir sistem değil. `McpServerKind::Plugin` etiketi, `mcp_servers` kaydı, imzalı manifest (signed/allowlisted paket), `capability_allowlist` (capability sandbox profili), `artifact_hash` sabitleme (kilitleme/sürüm), `/mcp revoke` + kill switch (tek tıkla devre dışı) — hepsi yukarıdaki tek mekanizmanın parçası. Canlı sandbox çalıştırma aynı Faz-3 dürüst sınırına tabi.
- [ ] Remote/mobile yalnız explicit device pairing, public key, nonce/replay koruması, revoke, bağlantı şifreleme ve server-side kill switch'ten sonra ele alınır. **F10'a taşındı** (kullanıcı 22 Ağustos 2026: "mobili F10'a at"); [[jarvis-f10-pentest-coding-deepening]] ile birlikte.
- [ ] Cross-device handoff: task scope'u genişletmeyen typed handoff, offline queue/conflict policy ve alıcı cihazda yeniden policy değerlendirmesi. **Mobil ile birlikte F10'a taşındı.**

Tamamlanma ölçütü: Yerel desktop sürümü güvenilir olmadan hiçbir eklenti, entegrasyon veya remote cihaz tool yetkisi ya da kişisel bellek erişimi almaz. **(F8 mobil hariç 22 Ağustos 2026'da bu ölçütle tamamlandı: MCP egress + eklenti güven çekirdeği 5/6 faz gerçek testle, yerel takvim çalışıyor; Faz-3 canlı taşıma + mobil F10'a taşındı.)**

### F9 — Operasyonel olgunluk, release ve uzun dönem bakım

Durum: TAMAMLANDI (21 Ağustos 2026) — 7/7 madde. Kullanıcı F8'den ÖNCE F9'u seçti (F8'in remote/mobile kapısı "güvenilir desktop" gerektiriyor, o da büyük ölçüde F9). İlk parça: release pipeline'ın "migration kontrolü" alt maddesi — `SqliteStore::verify_schema_migrations` şema göç kayıtlarının bütünlüğünü doğruluyor (en yüksek sürüm == CURRENT_SCHEMA_VERSION, 1..=güncel arası boşluk yok, her göçün adı dolu). Bu, bu oturumda her yeni tabloda elle hatırlanması gereken "sürüm artırıldı ama migration satırı unutuldu" hata sınıfını otomatik yakalıyor; `cargo test`'in (dolayısıyla release kapısının) parçası. Gerçek testle kanıtlandı: silinen bir göç satırı yakalanıyor (kontrolün sessizce her şeye 'tamam' demediğinin kanıtı).

- [x] Release pipeline: format, test, clippy, dependency/security denetimi, release build, migration kontrolü ve E2E smoke'u tek raporda birleştirme.
  - **Kanıt:** `scripts/release_check.sh` artık 7 adımlı, sürüm-damgalı tek birleşik rapor basıyor (format → bağımlılık kilidi → test[+migration bütünlüğü+audit witness] → clippy → release build → MCP smoke → loopback health). Herhangi bir adım başarısız olursa `set -e` ile durup PASS satırını hiç basmıyor. Migration bütünlük kontrolü `cargo test`'in parçası. Bağımlılık güvenlik denetimi (`cargo audit`) bilinçli olarak AYRI (`scripts/security_audit.sh`, internet erişir) — çevrimdışı gate'in çevrimdışı garantisini bozmamak için; birleşik rapor sonunda ona işaret ediyor.
- [x] Metrikler (gizlilik-güvenli): başarı/verification oranı, policy kararları, capability dağılımı, uyarı sayısı, ortalama işlem süresi (latency).
  - **Kanıt:** `Runtime::metrics_summary` → `RuntimeMetricsSummary`: toplam görev, capability bazında görev sayımı, verification pass/fail, policy allow/deny/ask, uyarı olayı sayısı. Kaynak veri `StructuredLogEvent` zaten yalnız olay adı + ID taşıdığı için özet YALNIZ sayım ve capability/olay adı içeriyor — hiçbir kullanıcı metni/hedef/sır girmiyor (F9'un "kişisel içerik toplamadan yerel telemetry" şartı). Hiçbir yere gönderilmiyor, yalnız kullanıcının kendi sistemini gözlemlemesi için. Gerçek testle kanıtlandı: gerçek görevler özete yansıyor, boş runtime sıfır veriyor, Debug çıktısında istek kimlikleri sızmıyor.
  - **Tamamlandı:** ortalama işlem süresi (latency) `handle_with_resolution` zamanlamasıyla eklendi; `/metrics` TUI komutu bağlandı (`format_metrics_summary`). Anlık CPU/RAM/disk zaten `system.health` capability'sinden geliyor. Token-üretim hızı ölçümü (model çağrısına özel) ileride model katmanına eklenebilir — çekirdek operasyonel metrikler tam.
- [x] Gerçek timeout/cancellation worker: process group, cleanup handles, resource quota, watchdog ve stuck-task recovery.
  - **Kanıt (çoğu F4'ten mevcuttu, bu turda process-group boşluğu kapatıldı):** watchdog (`wait_with_timeout` — süre aşınca SIGTERM→SIGKILL, testli), kooperatif iptal (`CancelFlag`, `Runtime::cancel`), kaynak kotası (F4 cgroup v2 + rlimits, canlı OOM-kill'le kanıtlı), ve stuck-task recovery (`recover_interrupted_tasks` — açılışta RUNNING→INTERRUPTED, bağlı+testli) zaten vardı. Bu turda **process group** eklendi: worker artık kendi süreç grubunda başlatılıyor (`process_group(0)`, hem test hem üretim dalı) ve `terminate_then_kill` grup lideri olan bir çocuğu öldürürken TÜM GRUBU hedefliyor — böylece worker'ın doğurduğu alt-süreçler (cargo→rustc gibi) yetim kalmıyor. Grup-öldürme `getpgid` ile koşullu: yalnız gerçekten grup lideri olan çocuklar için (aksi halde `kill(-pid)` JARVIS'in kendi grubunu vurabilirdi — güvenlik için düz çocuklarda doğrudan öldürme).
  - **Test:** Yeni `a_timed_out_worker_group_leaves_no_orphaned_child_processes` — kendi grubunda bir kabuk içinde bir alt-süreç fork edip süre aşımıyla öldürüldükten sonra grubun TAMAMEN boş olduğunu (`kill(-pgid, 0)` → ESRCH) kanıtlıyor; grup-öldürme olmadan fork edilen süreç hayatta kalırdı. Mevcut SIGTERM→SIGKILL testleri bozulmadı (koşullu grup-öldürme güvenli).
- [x] Backup/retention komutları + restore tatbikatı + audit export/witness.
  - **Kanıt:** `SqliteStore::create_verified_backup` — `backup_to`'nun (atomik VACUUM INTO) üstüne üç şey ekliyor: (1) **restore tatbikatı** — yedek alınır alınmaz AÇILIP doğrulanıyor (şema göç bütünlüğü + audit hash-zinciri); geçmezse yedek BOZUK sayılıp siliniyor. Yani her yedeğin "gerçekten geri yüklenebilir" olduğu, ona güvenmeden ÖNCE kanıtlanıyor — sessizce bozuk bir yedek biriktirmek felaket anında hiç yedek olmamaktan beterdir. (2) Zaman damgalı isim (üzerine yazmaz). (3) Saklama (retention): en yeni N yedek tutulur, eskiler prune edilir. Gerçek testle kanıtlandı: yedek gerçekten canlı verinin kopyasını içeriyor ve yeniden açılabiliyor; retention en yeni N'i tutuyor; kurcalanmış audit zincirli bir yedek doğrulamayı geçemiyor.
  - **Tamamlandı:** `/backup` TUI komutu (doğrulanmış yedek + saklama), `/audit-export <dosya>` komutu (`export_audit_chain`: hash-zincirini JSONL witness olarak dışa aktarır, önce bütünlük doğrular, kurcalanmışı reddeder). config/model/dataset rollback zaten mevcut (`/dataset mark`, `/model-runs`, migration-öncesi otomatik yedek). Gerçek testle kanıtlandı: witness round-trip + var-olan-dosya reddi + kurcalanmış-zincir reddi.
- [x] Sürüm/migration yönetimi: semantic version + changelog + compatibility check.
  - **Kanıt:** `migrate()` artık açılışta bir uyumluluk kontrolü yapıyor: diskteki veritabanı BU binary'nin bildiği `CURRENT_SCHEMA_VERSION`'dan DAHA YENİ bir şema taşıyorsa (daha yeni bir JARVIS yazmış, kullanıcı eski sürüme dönmüş), açmayı REDDEDİYOR — eski bir binary'nin anlamadığı yeni şemaya yazıp veriyi bozmasını önlüyor (down-migration olmadığı için sessizce geçmek tehlikeli olurdu). Gerçek testle kanıtlandı: gelecekten bir migration satırı eklenmiş bir DB reddediliyor, aynı sürüm sorunsuz açılıyor.
  - **Tamamlandı:** `JARVIS_VERSION` (Cargo.toml'dan, tek kaynak) `/status`'ta gösteriliyor; `CHANGELOG.md` eklendi (Semantic Versioning). Config migration akışı, şema migration mekanizmasının (`migrate()` + versiyonlu göç kayıtları) aynısı — zaten mevcut ve F9 madde 1'de bütünlük kontrolüyle korunuyor.
- [~] Güvenlik bakım döngüsü — bağımlılık güvenlik denetimi + güncelleme tamam (threat-model review + responsible disclosure kanalı hâlâ açık).
  - **Kanıt:** `cargo audit` (RustSec) ile JARVIS'in kendi bağımlılıkları denetlendi — bir güvenlik aracının kendi tedarik zincirinde bilinen açık taşımaması için. İlk denetim 2 yüksek-önemli açık (quick-xml 0.30 DoS) + 4 bakım uyarısı buldu. `cargo update` ile semver-uyumlu güncellemeler yapıldı (496+78+9 test + offline release build hâlâ geçiyor). Kalan uyarılar transitif pinlemeler nedeniyle otomatik çözülemedi; her biri gerekçesiyle `.cargo/audit.toml`'a kabul olarak eklendi (sessiz bastırma DEĞİL — YENİ açıklarda kapı hâlâ başarısız olur) ve [docs/security_dependency_audit.md](docs/security_dependency_audit.md)'de belgelendi. `scripts/security_audit.sh` (çevrimdışı release kapısından ayrı, çünkü advisory DB indirir) bu denetimi tekrarlanabilir kılıyor.
  - **Not:** quick-xml DoS'u masaüstü erişilebilirlik (atspi/zbus_xml) yığınından geliyor; ayrıştırılan XML yerel D-Bus verisi, saldırgan kontrolünde değil — gerçek maruziyet düşük. secret scanning ve pentest bulgu takibi zaten mevcut (F3 sır filtresi + F7.6 bulgu kaydı).
  - **Kalan:** threat-model review dokümantasyonu, responsible disclosure kanalı.
- [x] Kullanıcı kabul/release gate: offline çalışma, veri silme/export, model kapatma, erişilebilirlik, Türkçe UX ve performans hedefleri için checklist.
  - **Kanıt:** [docs/release_acceptance_checklist.md](docs/release_acceptance_checklist.md) — her kabul kriterini onu KANITLAYAN somut bir teste/komuta bağlayan bir checklist (slogan değil). Otomatik kapı (`release_check.sh` + `security_audit.sh`) + elle gözden geçirilen maddeler (Türkçe UX, erişilebilirlik, sürüme-özel performans hedefleri). Çevrimdışı çalışma, veri silme/export, model kapatma, sürüm/yedek/geri-alma, timeout/recovery, gizlilik-güvenli metrik — hepsi mevcut teste/komuta işaret ediyor.
