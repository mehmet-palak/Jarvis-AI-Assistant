...
- [x] Aktif keşif (yalnız scope onaylıysa) — port/servis tarama (kalan alt maddeler açık: subdomain brute-force, JS analiziyle endpoint keşfi).
  - **Kanıt:** `Runtime::scan_pentest_ports` gerçek TCP bağlantı denemeleriyle çalışıyor — sahte/simüle değil, gerçek dinleyen bir portu "açık", dinlemeyeni "kapalı" olarak raporladığı gerçek testle kanıtlandı. Hedefi gerçekten dokunduğu için (yalnız sorgulamayan pasif keşiften farklı olarak) F7.1'in AYNI tek giriş noktasından (`authorize_pentest_action`) `PentestMode::Active` talep ederek geçiyor — yalnız SAFE'e izin veren bir scope bunu reddediyor, tıpkı başka bir aktif eylemi reddedeceği gibi.
  ...
  - **Test disiplini:** Süre-dolumu davranışı, gerçek ama yavaşça başarısız olan bir port beklemek yerine (ki bu network zamanlamasına bağlı olur, hiç deterministik olmazdı) `pentest_network_gate`'in kendi testlerindeki desenle aynı şekilde test edildi — zaten süresi dolmuş bir deadline doğrudan veriliyor.
  - **Kalan:** Bu, F7.2'nin SOCKS5 kapısından DEĞİL, doğrudan JARVIS'in kendi sürecinden bağlanıyor — henüz sandbox'lı bir pentest worker'ı olmadığı için (bu, F7.2'nin kendi kalan maddesi). Servis parmak izi/banner alma burada yok, ayrı bir alt madde.
  - **Test:** 7 yeni test (gerçek açık/kapalı port raporlama, mod tavanı reddi, scope dışı hedef reddi, boş liste reddi, port tavanı reddi, deadline-dolunca-durma, deadline-dolmamışken-durmama).
- [ ] Subdomain brute-force, JS analiziyle endpoint keşfi.
